...
class List
{
    ...
    void insert(string name, int age, int year_joining)
    {
        ...
        else if(head->year_joining>year_joining)
        {
            Link* new_Node = new Link;
            put_data(new_Node,name,age,year_joining);
            new_Node->next = head;
        ...
        else
        {
            ...
            while(current_Node&&current_Node->year_joining<year_joining)
            {
...
int main()
{
    ...
    obj.insert("double",105,2000);
    obj.insert("ptr",20,2019);
    //obj.display_head();
    obj.insert("int",70,1990);
    //obj.insert("byte",90,1990);
